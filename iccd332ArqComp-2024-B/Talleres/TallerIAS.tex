...
\addbibresource{/home/josetm-epn/MI-REPO/EPN-Lectures/iccd332ArqComp-2024-B/bibliography/bibliography.bib}
\author{Anthony Erazo, Dylan Zuñiga, Joset Muela}
\date{2026-06-12}
\title{Taller — Máquina IAS / Von Neumann\\\medskip
\large ICCD332 Arquitectura de Computadores}
\hypersetup{
 pdfauthor={Anthony Erazo, Dylan Zuñiga, Joset Muela},
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|cccc|}
    ...
    00001100 & 0x0C       &  DIV M(X)         & $[MQ]\leftarrow[AC]\div[X], [AC]\leftarrow\text{resto}$\\
    00001101 & 0x0D       &  JUMP M(X,0:19)   & $[PC]\leftarrow X\text{ (izq)}$\\
    00001110 & 0x0E       &  JUMP M(X,20:39)  & $[PC]\leftarrow X\text{ (der)}$\\
    00001111 & 0x0F       &  JUMP+ M(X,0:19)  & $\text{if } [AC] \ge 0 \text{ then } [PC]\leftarrow X\text{ (izq)}$\\
    00010000 & 0x10       &  JUMP+ M(X,20:39) & $\text{if } [AC] \ge 0 \text{ then } [PC]\leftarrow X\text{ (der)}$\\
    00010100 & 0x14       &  LSH              & $[AC]\leftarrow[AC]\times 2$\\
    00010101 & 0x15       &  RSH              & $[AC]\leftarrow[AC]\div 2$\\
    00010010 & 0x12       &  STOR M(X,8:19)   & $[X\text{ (dir. izq)}]\leftarrow[AC](28:39)$\\
    00010011 & 0x13       &  STOR M(X,28:39)  & $[X\text{ (dir. der)}]\leftarrow[AC](28:39)$\\
    \hline
  \end{tabular}
\end{table}
...
siguientes preguntas considerando la Tabla \ref{tab:org4bb7dd4} como el
mapa de memoria de la máquina y obtenga el programa en lenguaje ensamblador y en
RTL.

\begin{table}[htbp]
\caption{\label{tab:org4bb7dd4}Mapa de Memoria}
\centering
\begin{tabular}{rrrrr}
\hline
...
\item \([AC] \leftarrow 0x0000000005-0x0000000002\)
\end{itemize}
\end{enumerate}

\subsubsection{Código RTL}
\label{sec:org9ea9cce}
...
\item \([AC] \leftarrow -[940]\)
\item \([940] \leftarrow [AC]\)
\item \([IR] \leftarrow HALT\)
\end{itemize}
\end{itemize}


\subsubsection{Código Assembler}
\label{sec:org025bbbc}
...
\hline
Joset Muela & joset.muela@epn.edu.ec & lider\\[0pt]
Anthony Erazo & anthony.erazo01@epn.edu.ec & colaborador\\[0pt]
Dylan Zuñiga & dylan.zuniga@epn.edu.ec & colaborador\\[0pt]
\hline
\end{tabular}
\end{center}

\section{Verificación de Entregables [100\%]}
\label{sec:orgf01c362}
